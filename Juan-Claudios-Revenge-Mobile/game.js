...
// Sounds (fail gracefully if missing)
const sounds = {
...
// ------------------------------------------------------------
// MOBILE SWIPE + TAP CONTROLS (Continuous Movement)
// ------------------------------------------------------------
if (isMobile) {
  ...
  canvas.addEventListener("touchmove", (e) => {
    ...
    // Medium sensitivity: 40px threshold
    if (deltaX > 40) {
      keys.right = true;
      keys.left = false;
    } else if (deltaX < -40) {
...
function update() {
  ...
  if (gameState === "playing") {
    ...
    enemies.forEach((e) => {
      ...
      if (e.x <= 10 || e.x + e.width >= canvas.width - 10) hitEdge = true;
    });
    ...
    if (enemyStepDown) {
      enemies.forEach((e) => {
        if (!e.alive) return;
        e.y += 10;
        if (e.y + e.height >= ship.y) endGame();
      });
      enemyStepDown = false;
    }
    ...
    bullets.forEach((b) => {
      if (
        ...
        if (boss.hp <= 0) {
          ...
          gameState = "levelComplete";
          setMessage(
            "Boss defeated! Press ENTER for the next wave.",
            "#29c76f"
          );
          try {
            sounds.levelUp.play();
